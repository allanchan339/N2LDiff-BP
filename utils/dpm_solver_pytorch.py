        ...
        ***
        ...
        if schedule == 'discrete':
            ...
            self.t_array = torch.linspace(
                0., 1., self.total_N + 1)[1:].reshape((1, -1)).to(dtype=dtype)
...
class DPM_Solver:
    ...
    def dynamic_thresholding_fn(self, x0, t):
        ...
        s = expand_dims(torch.maximum(
            s, self.thresholding_max_val * torch.ones_like(s).to(s.device)), dims)
    ...
    def add_noise(self, x, t, noise=None):
        ...
        xt = expand_dims(alpha_t, x.dim()) * x + \
